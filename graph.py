...
#-> Cycle Graph C8
def cycle_graph_c8(show: bool = False):
    G = nx.cycle_graph(8)
    if show:
...
# Path Graph P16
def path_graph_p16(show: bool = False):
    G = nx.path_graph(16)
    if show:
...
def complete_bipartite_graph_k88(show: bool = False):
...
#-> Complete Bipartite Graph K8,8
def complete_bipartite_graph_k_nn(n, show: bool = False):
...
# Star Graph S16
def star_graph_s16(show: bool = False):
    G = nx.star_graph(16)
    if show:
...
def regular_graph_4_8(show: bool = False):
...
def cubic_graph_3_16(show: bool = False):
...
def disjoint_union_c4(show: bool = False):
...
def complete_bipartite_graph_k1616(show: bool = False):
...
def hypercube_graph_q5(show: bool = False):
...
def tree_graph_8(show: bool = False):
...
# Wheel Graph W16
def wheel_graph_w16(show: bool = False):
    G = nx.wheel_graph(16)
    if show:
        plt.figure(figsize=(8, 8))
        pos = nx.circular_layout(G)
...
def random_connected_graph_16(p=0.15, show: bool = False):
    #n, p = 16, 0.25
    n=16
    while True:
...
def expander_graph_32(show: bool = False):
...
def expander_graph_n(n, show: bool = False):
...
def planar_connected_graph_16(show: bool = False):
